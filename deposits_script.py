...
def get_category_type(input_str, error_count):
    ...
    if input_str not in cache:
        ...
        data = {'prompt': "The following closed set of categories are business income types and only select from these categories, where \"Ask My Accountant\" is default if none are selected. :\n\n \"%s\" \n\n \"%s\"\nCategory:"%(categories, input_str), 'max_tokens': 1000, 'top_p': 1.0, 'stream': False, 'temperature': 0.0, 'frequency_penalty': 0.0, 'presence_penalty':0.0}
...
# Main program
with open("transactions.csv") as csv_file, open("deposits_tab.csv", "w") as output_file:
